...
$title = $old['title'] ?? '';
$content = $old['content'] ?? '';
$sourceLanguage = $old['source_language'] ?? 'tr';
      ...
      <div class="card overflow-hidden">
        <form method="post" action="/actions/create_article.php" class="p-5 sm:p-8">
          ...
          <div class="flex items-center gap-3 mb-4">
            <label class="text-muted text-sm">Kaynak dil</label>
            <select name="source_language" class="bg-surface-3 border border-border rounded-lg px-3 py-1.5 text-sm">
              <?php foreach (ARTICLE_LANGUAGES as $code => $name): ?>
                <option value="<?= h($code) ?>" <?= $sourceLanguage === $code ? 'selected' : '' ?>><?= h($name) ?></option>
              <?php endforeach; ?>
            </select>
          </div>
          ...
          <div class="flex items-center justify-between mt-4 pt-4 border-t border-border gap-3">
            ...
            <button type="submit" class="btn-primary text-sm px-5 py-2">Oluştur ve Çeviriye Aç</button>
          </div>
          <?php if ($error): ?>
