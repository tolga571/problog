...
$stmt = db()->prepare("SELECT * FROM posts WHERE id = ? AND post_type = 'article'");
$stmt->execute([$postId]);
$post = $stmt->fetch();

if (!$post || ($post['status'] !== 'published' && (!$user || $user['id'] !== $post['author_id']))) {
...
$requestedLang = (string) ($_GET['lang'] ?? $post['source_language']);
if (!isset(ARTICLE_LANGUAGES[$requestedLang])) {
    $requestedLang = $post['source_language'];
}

$isSource = $requestedLang === $post['source_language'];
$translation = get_post_translation($postId, $requestedLang);
$sentences = $translation['sentences'] ?? [];

$sourceTranslation = $isSource ? $translation : get_post_translation($postId, $post['source_language']);
...
$availableLangStmt = db()->prepare('SELECT language_id FROM post_translations WHERE post_id = ?');
$availableLangStmt->execute([$postId]);
$availableLangs = array_column($availableLangStmt->fetchAll(), 'language_id');
if (!in_array($post['source_language'], $availableLangs, true)) {
    $availableLangs[] = $post['source_language'];
}
        ...
        <select name="lang" onchange="this.form.submit()" class="bg-surface-3 border border-border rounded-lg px-2 py-1 text-xs">
          <?php foreach (ARTICLE_LANGUAGES as $code => $name): ?>
            <option value="<?= h($code) ?>" <?= $requestedLang === $code ? 'selected' : '' ?> <?= !in_array($code, $availableLangs, true) ? 'disabled' : '' ?>>
              <?= h($name) ?><?= $code === $post['source_language'] ? ' (Kaynak)' : '' ?>
            </option>
          <?php endforeach; ?>
        </select>
      </form>
