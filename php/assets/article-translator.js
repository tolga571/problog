...
class ArticleTranslator {
  ...
  addSentenceRow(sentence, focus = true) {
    ...
    row.innerHTML = `
      ...
      <div class="flex-1 min-w-0">
        ${!this.isSource && sourcePreview ? `<p class="text-muted-2 text-xs mb-1.5 italic">${escapeHtml(sourcePreview.text)}</p>` : ''}
    ...
    this.views.set(code, { view, note: row.querySelector('.translator-note'), wrap: row });
  ...
  collectSentences() {
    return [...this.rowsEl.querySelectorAll('.translator-row')].map((row, i) => {
      ...
      return { code, sort: i + 1, p, text, note: entry?.note?.value?.trim() || '' };
    });
  }
  ...
  importJson() {
    ...
    sentences.forEach((item) => {
      ...
      if (entry?.view) {
        ...
        this.addSentenceRow({ code: item.code, sort: this.views.size + 1, p: 0, text: item.text || '', note: item.note || '' }, false);
      }
    });
